...
import { assignCaseToAgronomist, fetchAgronomists, type AgronomistUserRow } from '../api/realApi';
...
export function CaseTableEnhanced({ cases }: { cases: CaseManagementCaseRow[] }) {
  ...
  const [agronomists, setAgronomists] = useState<AgronomistUserRow[]>([]);
  const [agronomistsLoading, setAgronomistsLoading] = useState(false);
  const [agronomistsError, setAgronomistsError] = useState<string | null>(null);
  const [selectedAgronomistId, setSelectedAgronomistId] = useState<string>('');
  ...
  const agronomistOptions = useMemo(() => {
    return agronomists
      .filter((a) => a?.id)
      .map((a) => {
        const countyLabel = a.county ? ` — ${a.county}` : '';
        const phoneLabel = a.phone ? ` (${a.phone})` : '';
        return { value: a.id, label: `${a.name}${countyLabel}${phoneLabel}` };
      });
  }, [agronomists]);

  useEffect(() => {
    let cancelled = false;
    if (!assignModalCase || !canAssignAgronomist) return;

    // Reset form each time modal opens
    setSelectedAgronomistId('');
    setAssignmentNotes('');
    setSendNotification(true);
    setAgronomistsError(null);

    (async () => {
      setAgronomistsLoading(true);
      try {
        const rows = await fetchAgronomists();
        if (cancelled) return;
        setAgronomists(rows);
      } catch (e: any) {
        if (cancelled) return;
        setAgronomists([]);
        setAgronomistsError(e?.message ? String(e.message) : 'Failed to load agronomists.');
      } finally {
        if (!cancelled) setAgronomistsLoading(false);
      }
    })();
        ...
        >
          ...
          >
              ...
              <div className="mb-6">
                ...
                {agronomistsError ? (
                  <div
                    ...
                    style={{ fontFamily: 'IBM Plex Sans, sans-serif', color: '#DC2626' }}
                  >
                    {agronomistsError}
                  </div>
                ) : null}
                <select
                  ...
                  disabled={agronomistsLoading || assignSubmitting}
                ...
                >
                  <option value="" disabled>
                    {agronomistsLoading ? 'Loading agronomists…' : 'Choose an agronomist...'}
                  </option>
                  {agronomistOptions.map((opt) => (
            ...
            >
              ...
              <button
                onClick={() => {
                  if (!selectedAgronomistId || assignSubmitting) return;
                  (async () => {
                    try {
                      setAssignSubmitting(true);
                      await assignCaseToAgronomist({
                        caseId: assignModalCase.id,
                        assigned_agronomist: selectedAgronomistId,
                        notes: assignmentNotes,
                        notify: sendNotification,
                      });
                      setAssignSuccess({ caseId: assignModalCase.id, smsSent: sendNotification });
                      setAssignModalCase(null);
                    } catch (e) {
                      // Keep the modal open; surface a simple error message.
                      setAgronomistsError(
                        e instanceof Error ? e.message : 'Failed to assign case. Please try again.'
                      );
                    } finally {
                ...
                disabled={!selectedAgronomistId || assignSubmitting}
                className="px-4 py-2 rounded-lg transition-colors hover:opacity-90 flex items-center gap-2 disabled:opacity-60"
                style={{
                  backgroundColor: '#2D6A4F',
                  color: '#FFFFFF',
                  fontFamily: 'IBM Plex Sans, sans-serif',
              ...
              >
                ...
                {assignSubmitting ? 'Assigning…' : 'Assign Case'}
              </button>
            </div>
          </div>
        </div>
