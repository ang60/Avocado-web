...
from .models import QuarantineManagement, KephisThresholdSetting, QuarantineActionLog, ChinaFarmCertification
from .serializers import (
    QuarantineManagementSerializer,
    KephisThresholdSettingSerializer,
    QuarantineActionLogSerializer,
    ChinaFarmCertificationSerializer,
)
...
@extend_schema(tags=['KEPHIS China Export'])
class ChinaFarmCertificationViewSet(viewsets.ModelViewSet):
    """
    Issue China-facing Farm IDs after inspection and store KEPHIS management insights.
    """

    queryset = ChinaFarmCertification.objects.select_related('farmer', 'farmer__user', 'inspector').all()
    serializer_class = ChinaFarmCertificationSerializer
    permission_classes = [permissions.IsAuthenticated]
    filter_backends = [filters.SearchFilter]
    search_fields = ['china_farm_id', 'farmer__name', 'farmer__farm_name', 'farmer__county', 'farmer__user__phone_number', 'status']
    ...
    def create(self, request, *args, **kwargs):
        if not self._can_write(request.user):
            raise PermissionDenied('Only KEPHIS/Admin users can issue China Farm IDs.')
        return super().create(request, *args, **kwargs)

    def update(self, request, *args, **kwargs):
        if not self._can_write(request.user):
            raise PermissionDenied('Only KEPHIS/Admin users can update China Farm IDs.')
    ...
    def issue(self, request, pk=None):
        ...
        cert.status = 'issued'
        cert.inspector = request.user
        if not cert.inspected_at:
            cert.inspected_at = timezone.now()
        cert.save()
        return Response(ChinaFarmCertificationSerializer(cert).data)

    @action(detail=True, methods=['post'])
    def approve_export(self, request, pk=None):
        """
        Approve a farm for export after China Farm ID issuance.
        Also marks the underlying FarmerProfile as export-ready.
        """
        cert = self.get_object()
        if not self._can_write(request.user):
            raise PermissionDenied('Only KEPHIS/Admin users can approve exports.')
        if cert.status != 'issued':
            raise PermissionDenied('Export approval requires an issued China Farm ID.')
        if not cert.china_farm_id:
            cert.ensure_china_farm_id()

        cert.export_approved_at = timezone.now()
        cert.export_approved_by = request.user
        cert.save(update_fields=['china_farm_id', 'export_approved_at', 'export_approved_by', 'updated_at'])

        try:
            farmer = cert.farmer
            # Mark as export-ready in farmer registry (used across dashboards).
            farmer.export_eligibility = 'ready'
            farmer.save(update_fields=['export_eligibility'])
        except Exception:
            pass
